export default class Enemy {
	 ...
	 */
	move() {
		let distance = this.computeDistance(this.tile, this.player.tile); // the distance between these two tiles
		if (distance > 5) {
			return; //i.e. do nothing
		}
		if (distance <= 1) {
			this.attack(); // enemy attacks
			return; // no movement if attack. it's either or situation, attack or move
		}
		let chosen = this.getTile();
		// as long as chosen is not equal to null ( as long as chosen = tile), just move ourselves there)
		if (chosen != null) {
			this.row = chosen.y;
			this.col = chosen.x;
			this.x = this.col * 8;
			this.y = this.row * 8;
			// update the tile (update the data related to where we are)
			this.tile = this.map.get(this.col, this.row);
		}
	}
	/**
	 ...
	 */
	// gets closest tile to player of the four that are around you
	getTile() {
		let dirs = [];
